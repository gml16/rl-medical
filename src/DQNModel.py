...
class RNNetwork3D(nn.Module):

    def __init__(self, agents, frame_history, number_actions, xavier=True, logger=None):
        super(RNNetwork3D, self).__init__()
...
class DQN:
    ...
    def __init__(
        ...
        if type == "Network3d":
            ...
            self.target_network = Network3D(
                ...
                self.device)
        elif type == "RNNetwork3d":
            self.q_network = RNNetwork3D(
                agents,
                frame_history,
                number_actions,
                logger=self.logger).to(
                self.device)
            self.target_network = RNNetwork3D(
                agents, frame_history, number_actions,logger=self.logger).to(
                self.device)
            ...
            self.target_network = CommNet(
                ...
                attention=attention).to(
                self.device)
        elif type == "CommNetWithCommunicationCutoff":
            self.q_network = CommNetWithCommunicationCutoff(
                agents,
                frame_history,
                number_actions,
                attention=attention).to(
                self.device)
            self.target_network = CommNetWithCommunicationCutoff(
                agents,
                frame_history,
                number_actions,
                attention=attention).to(
    ...
    def _calculate_loss(self, transitions, discount_factor):
        ...
        # Bellman equation
        #self.logger.log(rewards.shape)
        #self.logger.log(isNotOver.shape)
        #self.logger.log(max_target_net.shape)
