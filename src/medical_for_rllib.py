...
class MedicalPlayer(gym.Env):
    ...
    def __init__(self,  config: EnvContext):
        ...
        self.oscillations_allowed = config["oscillations_allowed"]
        ...
        self.adj = config["adj"]
        self.physical_reward = config["physical_reward"]
        self.beta = config["beta"]
        self.reward_limiter = config["reward_limiter"]
        ...
        if file_type == "brain":
            self.files = filesListBrainMRLandmark(config["files_list"],
                                                  returnLandmarks,
                                                  self.agents)
        elif file_type == "cardiac":
            self.files = filesListCardioLandmark(config["files_list"],
                                                 returnLandmarks,
                                                 self.agents)
        elif file_type == "fetal":
            self.files = filesListFetalUSLandmark(config["files_list"],
                                                  returnLandmarks,
                                                  self.agents)

        # prepare file sampler
        self.filepath = None
        self.sampled_files = self.files.sample_circular(config["landmark_ids"])
        self.fixed_spawn = config["fixed_spawn"]
